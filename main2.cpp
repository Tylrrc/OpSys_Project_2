...
int main(int argc, char *argv[]) {
   ...
   if (argc < 2) {
        ...
        cout << "Usage: " << argv[0] << " <seed_int> " << endl;
        return 1;
   }

   //convert argv[1] to interger for randomization seed
   srand(atoi(argv[1]));

   //create three player objects and house them in an array
   Player* player_1 = new Player;
   Player* player_2 = new Player;
   Player* player_3 = new Player;
   Player* player[NUM_THREADS] = {player_1,player_2,player_3};


   //assign player IDs [1,2,3]
   for (int n = 0; n < NUM_THREADS; n++) {
      player[n]->id = n; //set player ID
   }

   // create an array of three threads
   pthread_t threads[NUM_THREADS];

   // initialize mutexes for deck and status
...
**/
void push_back_deck(Deck*& deck, const int card) {
